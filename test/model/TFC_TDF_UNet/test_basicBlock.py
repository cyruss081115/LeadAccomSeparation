...
class TestTDFBlock(unittest.TestCase):
    def test_init_other_activation(self):
        tdf_block = TDFBlock(channels=4, frequency_bins=4096, bottleneck=128, num_layers=1, activation='LeakyReLU')
...
class TestTDCBlock(unittest.TestCase):
    def test_init_other_activation(self):
        tdc_block = TDCBlock(in_channels=4, growth_rate=24, num_layers=1, kernel_size=3, activation='LeakyReLU')
        expected_architecture = nn.ModuleList([
            nn.Sequential(
                nn.BatchNorm1d(4),
    ...
    def test_forward_num_layers_greater_than_1(self):
        ...
        tdc_block = TDCBlock(in_channels=4, growth_rate=24, num_layers=3, kernel_size=3).to(device)
...
class TestTDSABlock(unittest.TestCase):
    def test_init_num_layers_equals_1(self):
        tdsa_block = TDSABlock(embed_dim=1024, num_heads=8, num_layers=1)
        expected_architecture = nn.ModuleList([
            nn.Sequential(
                nn.BatchNorm1d(1024),
    ...
    def test_init_num_layers_greater_than_1(self):
        tdsa_block = TDSABlock(embed_dim=1024, num_heads=8, num_layers=2)
        expected_architecture = nn.ModuleList([
            nn.Sequential(
                nn.BatchNorm1d(1024),
                nn.MultiheadAttention(1024, 8, dropout=0., bias=False),
    ...
    def test_forward_num_layers_equals_1(self):
        ...
        tdsa_block = TDSABlock(embed_dim=1024, num_heads=8, num_layers=1).to(device)
    ...
    def test_forward_num_layers_greater_than_1(self):
        ...
        tdsa_block = TDSABlock(embed_dim=1024, num_heads=8, num_layers=3).to(device)
        input = torch.rand([1, 4, 1378, 1024]).to(device) # [B, C, T, F]
        output = tdsa_block(input)
